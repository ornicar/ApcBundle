...
class ApcClearCommand extends ContainerAwareCommand
{
    ...
    /**
     * @param boolean $clearOpCodeCache
     */
    public function setClearOpCodeCache($clearOpCodeCache)
    {
        $this->clearOpCodeCache = $clearOpCodeCache;
    }
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('mode', null, InputOption::VALUE_OPTIONAL,
                'Clearing mode (possible choices are "fopen", "curl" or "goutte", e.g. -mode=fopen)', 'fopen')
    ...
    {
        $this->clearOpCodeCache = $input->getOption('opcode') || !$input->getOption('user');
        $this->clearUserCache = $input->getOption('user') || !$input->getOption('opcode');
    ...
    protected function validateMode()
    {
        ...
        if ($this->mode == 'goutte' && !class_exists('\Goutte\Client')) {
            throw new \InvalidArgumentException('Install Goutte Client by running composer install from the project root directory.');
        } else {
    ...
    protected function saveClearCacheScript()
    {
        ...
        $code = strtr(
            ...
            array(
                ...
                '%opcode%' => var_export($this->clearOpCodeCache, true)
            )
        );
